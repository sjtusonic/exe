...
int BestTravel::chooseBestSum(int t, int k, std::vector<int>& ls)
{
	...
	TIME::end = clock(); std::cout << "TIME(SEC) " << static_cast<double>(TIME::end - TIME::start) / CLOCKS_PER_SEC << "\n";
	...
	std::cout << "TIME(SEC) " << static_cast<double>(TIME::end - TIME::start) / CLOCKS_PER_SEC << "\n";
	return ans;
}
